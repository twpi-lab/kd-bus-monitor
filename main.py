...
def _crawl_store_and_notify_impl():
    ...
    # 1단계: 매칭된 공고 수집 (아직 발송 안 된 것만)
    matched = []  # [{"notice": dict, "is_urgent": bool}, ...]
...
def show_keywords():
    print(f"\n{'─'*60}")
    print(f"  🚌 BUS_KEYWORDS ({len(BUS_KEYWORDS)}개):\n     {', '.join(BUS_KEYWORDS)}")
    print(f"\n  📝 TENDER_KEYWORDS ({len(TENDER_KEYWORDS)}개):\n     {', '.join(TENDER_KEYWORDS)}")
...
def main():
    # 비대화형 1회 실행 (GitHub Actions·cron 용)
    if "--once" in sys.argv:
        crawl_store_and_notify()
        return
